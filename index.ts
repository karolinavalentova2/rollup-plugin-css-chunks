  ...
  return {
    ...
    load(id: string) {
      ...
      if (m !== null) {
        ...
        try {
          map = JSON.parse(
            Buffer.from(m[1], "base64").toString("utf-8").trim()
          );
        } catch (err) {
      ...
      for (const chunk of Object.values(bundle).reverse()) {
        ...
        if (pluginOptions.injectImports) {
          for (const c of chunk.imports) {
            if (bundle[c]) {
              code += (<OutputChunk>bundle[c]).imports
                .filter(filter)
                .map((f) => `@import '${f}';`)
                .join("");
            }
        ...
        for (const f of css_modules) {
          if (pluginOptions.sourcemap && emitFiles) {
            ...
            sources.push(
              ...css_data[f].map.sources.map((source) =>
                path.relative(
        ...
        const css_file_name = makeFileName(
          chunk.name,
          hash(code),
          chunk.isEntry
        ...
        chunk.code = chunk.code.replace(
          new RegExp(
            `CSS_FILE_${chunk.fileName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`,
            "g"
          ),
          css_file_url
        );
